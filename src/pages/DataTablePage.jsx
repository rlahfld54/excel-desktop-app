...
import { DateRangeFields, FormField, SearchButton } from '../components/common';
...
export default function DataTablePage() {
  ...
  const productCodeIndex = getColumnIndex(columns, [
    '품목 코드',
    '품목코드',
    '상품코드',
    '제품코드',
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows
      ...
      .filter(({ row }) => {
        ...
        const matchesDate =
          dateIndex < 0 ||
          ((!params.startDate || rowDate >= params.startDate) &&
            (!params.endDate || rowDate <= params.endDate));
        const customerValue =
          customerIndex >= 0 ? String(row[customerIndex] ?? '').toLowerCase() : '';
  ...
  const statusOptions = useMemo(() => {
    ...
    return [
      '전체',
      ...Array.from(new Set([...detected, '정상', '확인 필요', '반려', '승인 완료'])),
    ];
  }, [rows, statusIndex]);
  const ownerOptions = useMemo(() => {
    return currentUser.role === 'ADMIN' ? ['전체', ...serverOwnerOptions] : serverOwnerOptions;
    ...
    if (window.api?.querySalesData) {
      ...
      try {
        ...
        if (result?.ok && Array.isArray(data?.rows)) {
          setColumns(
            Array.isArray(data.columns) && data.columns.length > 0 ? data.columns : sqlColumns,
          );
          setRows(data.rows);
  ...
  return (
    <PageShell
      title="원본 데이터 조회"
      description="업로드한 원본 데이터를 조회하고, 반려 항목이 없는 데이터만 SQL에 저장합니다."
    >
      <div className="flex h-[calc(100vh-14rem)] flex-col">
        ...
        <section
          className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-xs dark:border-gray-700/60 dark:bg-gray-800"
          ...
          <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-4 py-3 dark:border-gray-700/60">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {totalRows.toLocaleString('ko-KR')}건 중{' '}
            ...
            {totalRows > params.pageSize && (
              <div className="flex items-center gap-2">
                <button
                  ...
                  type="button"
                  disabled={page <= 1 || queryAction === 'page'}
                ...
                <button
                  className="btn btn-secondary h-8 px-3 text-xs"
                  type="button"
                  disabled={page >= totalPages || queryAction === 'page'}
          ...
          <div className="min-h-0 flex-1 overflow-auto no-scrollbar">
            <table className="min-w-[1120px] w-full border-separate border-spacing-0 text-sm">
              <thead className="sticky top-0 z-10">
                <tr>
                  <th className="w-14 border-b border-r border-gray-200 bg-gray-50 px-2 py-2 text-center text-xs font-semibold text-gray-500 dark:border-gray-700/60 dark:bg-gray-900 dark:text-gray-400">
                    #
                  </th>
                  {columns.map((column) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {columns.map((column, cellIndex) => {
                        ...
                        const displayCell =
                          cellIndex === dateIndex ? formatTransactionDate(cell) : cell;
                        return (
                          <td
                            key={`${rowIndex}-${column}`}
                            className="h-8 max-w-64 truncate border-b border-r border-gray-200 px-3 py-1.5 text-gray-700 group-hover:bg-accent-50/60 dark:border-gray-700/60 dark:text-gray-200 dark:group-hover:bg-accent-500/10"
                            title={displayCell}
                          >
                            {cellIndex === statusIndex ? (
                              <span
                                className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${statusClass(cell)}`}
                              >
                                {cell}
                              </span>
                            ) : (
