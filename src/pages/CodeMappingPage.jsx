...
import { Modal, SearchButton, StatusBadge } from '../components/common';
          ...
          <thead className="sticky top-0 z-10">
            <tr>
              {columns.map((column) => (
                <th
                  key={column.label}
                  className="border-b border-r border-gray-200 bg-gray-50 px-3 py-2 text-left text-xs font-semibold text-gray-500 dark:border-gray-700/60 dark:bg-gray-900 dark:text-gray-400"
                >
                  {column.label}
                </th>
              ))}
...
export default function CodeMappingPage() {
  ...
  const metrics = useMemo(() => {
    ...
    return [
      {
        label: '거래처 기준',
        value: `${masterData.customers.length.toLocaleString('ko-KR')}건`,
        detail: '거래처코드·거래처명 기준',
      },
      {
        label: '제품 기준',
        value: `${masterData.products.length.toLocaleString('ko-KR')}건`,
        detail: `별칭 ${masterData.productAliases.length.toLocaleString('ko-KR')}건`,
      },
      {
        label: '단가 기준',
        value: `${activePriceCount.toLocaleString('ko-KR')}건`,
        detail: `검토 단가 ${reviewPriceCount.toLocaleString('ko-KR')}건`,
      },
      {
        label: '매핑 후보',
        value: `${pendingCount.toLocaleString('ko-KR')}건`,
        detail: `전체 별칭 ${aliasCount.toLocaleString('ko-KR')}건`,
      },
    ];
  }, [masterData]);
  ...
  const filteredAliases = customerAliases.filter((alias) => {
    ...
    const matchesSearch =
      !query ||
      [
        alias.sourceCustomerCode,
        alias.sourceCustomerName,
        alias.customerCode,
        alias.customerName,
      ].some((value) =>
        String(value ?? '')
  ...
  const candidateCustomers = masterData.customers.filter((customer) => {
    ...
    const selectable =
      ...
      (editingAlias && customer.customerCode === editingAlias.customerCode);
    return (
      selectable &&
      (customer.customerCode === aliasForm.customerCode ||
        !query ||
        [customer.customerCode, customer.customerName].some((value) =>
          String(value ?? '')
            .toLowerCase()
            .includes(query),
        ))
  ...
  const saveAlias = async () => {
    ...
    if (!aliasForm.sourceCustomerCode.trim() && !aliasForm.sourceCustomerName.trim())
      errors.source = '원본 거래처코드 또는 거래처명 중 하나를 입력해주세요.';
    ...
    try {
      ...
      setAliasErrors({
        form:
          /거래처|매핑|출처|상태/.test(message) && !/SQLITE_|SqliteError|database/i.test(message)
            ? message
            : '거래처 별칭 매핑을 저장하지 못했습니다.',
      });
    } finally {
  ...
  const toggleAliasStatus = async (alias) => {
    ...
    try {
      ...
      const result = await window.api.setCustomerAliasMappingStatus({
        mappingId: alias.mappingId,
        status,
      });
      ...
      const message =
        status === 'INACTIVE'
          ? '거래처 별칭 매핑을 미사용 처리했습니다.'
          : '거래처 별칭 매핑을 다시 사용하도록 변경했습니다.';
      setAliasMessage(message);
      addNotification({
        title: message,
        message,
  ...
  const downloadAliasTemplate = async () => {
    try {
      const currentData = hasLoadedMasterData
        ? masterData
        : await loadMasterData({ notify: false });
      ...
      if (error.name !== 'AbortError')
        setAliasMessage(`양식을 만들지 못했습니다: ${error.message}`);
    }
  };
  const selectAliasImportFile = async (event) => {
    ...
    setImportError('');
    try {
      const [parsed, latestData] = await Promise.all([
        parseSpreadsheetFile(file),
        loadMasterData({ notify: false }),
      ]);
      if (!latestData) throw new Error('기준 데이터를 불러오지 못했습니다.');
      const preview = previewCustomerAliasImport(
        parsed,
        latestData.customers,
        latestData.customerAliases,
      );
  ...
  const saveAliasImport = async () => {
    ...
    try {
      ...
      const message = error.message || '';
      setImportError(
        /행 ·|중복|거래처|업로드 출처|상태/.test(message) &&
          !/SQLITE_|SqliteError|database/i.test(message)
          ? message
          : '엑셀 파일에서 수정이 필요한 데이터가 발견되었습니다.',
      );
    } finally {
  ...
  return (
    <PageShell
      title="코드 매핑"
      description="등록된 거래처명, 제품명, 단가 기준과 매핑 후보를 확인합니다."
    >
      <div
        className="mb-4 flex gap-1 border-b border-gray-200 dark:border-gray-700"
        role="tablist"
        aria-label="코드 매핑 관리 영역"
      >
        {[
          ['masterData', '기준 데이터'],
          ['customerAliases', '거래처 별칭 매핑'],
        ].map(([id, label]) => (
          <button
            key={id}
            type="button"
            ...
            className={`border-b-2 px-4 py-3 text-sm font-semibold transition-colors ${activeSection === id ? 'border-teal-500 text-teal-700 dark:border-teal-400 dark:text-teal-200' : 'border-transparent text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-100'}`}
            onClick={() => {
              setActiveSection(id);
              if (id === 'customerAliases' && !hasLoadedMasterData && !isLoading)
                loadMasterData({ notify: false });
            }}
          >
            {label}
        ...
        <>
          <section className="mb-4 flex flex-wrap items-start justify-between gap-3 rounded-lg border border-gray-200 bg-white p-4 shadow-xs dark:border-gray-700/60 dark:bg-gray-800">
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-gray-500 dark:text-gray-400">
                엑셀에 입력된 거래처 코드와 거래처명을 등록된 기준 거래처에 연결합니다.
                <br />
                매핑은 로컬 SQLite에 저장됩니다.
              </p>
            </div>
          </section>
          ...
          <div className="mb-4 grid grid-cols-3 gap-3">
            {[
              ...
              ['사용 중', customerAliases.filter((item) => item.status === 'ACTIVE').length],
              ['미사용', customerAliases.filter((item) => item.status === 'INACTIVE').length],
            ].map(([label, count]) => (
              <div
                key={label}
          ...
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <input
              ...
              type="search"
              aria-label="거래처 별칭 검색"
              placeholder="원본 코드·이름 또는 기준 거래처 검색"
              value={aliasSearch}
            ...
            <select
              ...
              onChange={(event) => setAliasStatusFilter(event.target.value)}
            >
            ...
            <button
              ...
              onClick={() => {
                setAliasSearch('');
                setAliasStatusFilter('ALL');
              }}
            >
              초기화
            </button>
            <div className="flex flex-wrap gap-2 xl:ml-auto">
              <button className="btn btn-secondary" type="button" onClick={downloadAliasTemplate}>
                양식 다운로드
              </button>
              <label
                className={`btn btn-secondary cursor-pointer ${isParsingImport || !hasLoadedMasterData ? 'pointer-events-none opacity-50' : ''}`}
              >
                {isParsingImport ? '파일 읽는 중...' : '엑셀 일괄 등록'}
                <input
                  className="sr-only"
                  type="file"
                  accept=".xlsx,.csv"
                  onChange={selectAliasImportFile}
                  disabled={isParsingImport || !hasLoadedMasterData}
                />
              </label>
              <button className="btn btn-primary" type="button" onClick={() => openAliasModal()}>
                새 매핑 등록
              </button>
            </div>
          </div>
          {aliasMessage && (
            <p role="status" className="mb-3 text-sm text-gray-500 dark:text-gray-400">
              {aliasMessage}
            </p>
          )}

          {customerAliases.length === 0 ? (
            <section className="rounded-lg border border-gray-200 bg-white px-4 py-10 text-center dark:border-gray-700/60 dark:bg-gray-800">
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">
                등록된 거래처 별칭 매핑이 없습니다.
              </h3>
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                엑셀에서 사용하는 거래처명과 거래처코드를 기준 거래처에 연결해 관리할 수 있습니다.
              </p>
              <button
            ...
            <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white dark:border-gray-700/60 dark:bg-gray-800">
                ...
                <tbody>
                  {filteredAliases.length === 0 ? (
                    ...
                    filteredAliases.map((alias) => (
                      <tr key={alias.mappingId} className="group">
                        ...
                        <td className="border-b border-r border-gray-200 px-3 py-2 dark:border-gray-700/60">
                          {alias.customerCode} · {alias.customerName}
                        </td>
                        <td className="border-b border-r border-gray-200 px-3 py-2 dark:border-gray-700/60">
                          <StatusBadge status={alias.status} />
                        </td>
                        <td className="border-b border-r border-gray-200 px-3 py-2 dark:border-gray-700/60">
                          {new Date(alias.updatedAt).toLocaleDateString('ko-KR')}
                        </td>
                        <td className="whitespace-nowrap border-b border-gray-200 px-3 py-2 dark:border-gray-700/60">
                          <button
                            className="text-accent-700 hover:underline dark:text-accent-300"
                            type="button"
                            onClick={() => openAliasModal(alias)}
                          >
                            수정
                          </button>
                          <button
                            className="ml-3 text-gray-600 hover:underline dark:text-gray-300"
                            type="button"
                            onClick={() => toggleAliasStatus(alias)}
                            disabled={changingAliasId !== null}
                          >
                            {changingAliasId === alias.mappingId
                              ? '변경 중...'
                              : alias.status === 'ACTIVE'
                                ? '미사용'
                                : '다시 사용'}
      ...
      <Modal
        ...
        title="거래처 별칭 일괄 등록 검증"
        description={importPreview?.fileName ?? ''}
        size="4xl"
        onClose={() => {
          if (!isImporting) setImportPreview(null);
        }}
      >
        {importPreview && (
          <div className="space-y-4 p-5 text-sm">
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">
              {[
                ['전체 행', importPreview.totalCount],
                ['신규', importPreview.newCount],
                ['기존 매핑', importPreview.existingCount],
                ['경고', importPreview.warningCount],
                ['오류', importPreview.errorCount],
              ].map(([label, count]) => (
                <div key={label} className="rounded-md bg-gray-50 p-3 dark:bg-gray-900/50">
            ...
            <fieldset className="flex flex-wrap gap-4">
              <legend className="mb-2 font-semibold">기존 데이터 처리</legend>
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  name="alias-import-strategy"
            ...
            {importPreview.errorCount > 0 && (
              <p role="alert" className="font-semibold text-red-700 dark:text-red-300">
                오류가 있는 행을 수정한 파일로 다시 업로드해주세요. 오류가 있으면 전체 저장을
                진행하지 않습니다.
              </p>
            )}
            {importError && (
                ...
                <thead className="sticky top-0 bg-gray-50 dark:bg-gray-900">
                  <tr>
                    {[
                      ...
                      <th key={heading} className="border-b px-3 py-2">
                        {heading}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {importPreview.rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2">{row.sourceCustomerCode || '—'}</td>
                      <td className="px-3 py-2">{row.sourceCustomerName || '—'}</td>
                      ...
                      <td
                        className={`px-3 py-2 ${row.errors.length ? 'text-red-700 dark:text-red-300' : row.warnings.length ? 'text-amber-700 dark:text-amber-300' : 'text-teal-700 dark:text-teal-300'}`}
                      >
                        {[...row.errors, ...row.warnings].join(' / ') || '정상'}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="flex justify-end gap-2 border-t border-gray-200 pt-4 dark:border-gray-700">
              ...
              <button
                ...
                type="button"
                onClick={saveAliasImport}
                disabled={isImporting || importPreview.errorCount > 0}
              >
                {isImporting ? '저장 중...' : `${importPreview.totalCount}행 일괄 저장`}
              </button>
            </div>
          </div>
        )}
      ...
      <Modal
        open={aliasModalOpen}
        title={editingAlias ? '거래처 별칭 매핑 수정' : '거래처 별칭 매핑 등록'}
        description="원본 거래처 정보를 기준 거래처에 연결하고 로컬 SQLite에 저장합니다."
        size="lg"
        ...
        <div className="space-y-4 p-5 text-sm">
          <label className="block font-semibold text-gray-700 dark:text-gray-200">
            ...
            <select
              ...
              onChange={(event) =>
                setAliasForm((current) => ({ ...current, sourceType: event.target.value }))
              }
            >
            ...
            {aliasErrors.sourceType && (
              <span className="mt-1 block text-xs text-red-600">{aliasErrors.sourceType}</span>
            )}
          </label>
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block font-semibold text-gray-700 dark:text-gray-200">
              ...
              <input
                ...
                value={aliasForm.sourceCustomerCode}
                onChange={(event) =>
                  setAliasForm((current) => ({
                    ...current,
                    sourceCustomerCode: event.target.value,
                  }))
                }
                placeholder="예: 00123, DS-01"
              />
            </label>
            <label className="block font-semibold text-gray-700 dark:text-gray-200">
              원본 거래처명
              <input
                className="form-input mt-1 w-full"
                value={aliasForm.sourceCustomerName}
                onChange={(event) =>
                  setAliasForm((current) => ({
                    ...current,
                    sourceCustomerName: event.target.value,
                  }))
                }
                placeholder="예: (주)대성, 대성 본사"
              />
            </label>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                disabled={isLoading}
              >
                {isLoading ? '불러오는 중...' : '기준 데이터 불러오기'}
              </button>
            </div>
            <input
              id="alias-customer-search"
              className="form-input mt-1 w-full"
              type="search"
            ...
            <select
              ...
              size={Math.min(5, Math.max(candidateCustomers.length, 2))}
              aria-label="기준 거래처 선택"
              value={aliasForm.customerCode}
              onChange={(event) => {
                const customer = masterData.customers.find(
                  (item) => item.customerCode === event.target.value,
                );
                setAliasForm((current) => ({
                  ...current,
            ...
            {masterData.customers.length === 0 && (
              <p className="mt-1 text-xs text-gray-500">
                기준 데이터를 불러온 뒤 거래처를 선택하세요.
              </p>
            )}
            {aliasErrors.customer && (
              <p className="mt-1 text-xs text-red-600">{aliasErrors.customer}</p>
            )}
          </div>
          {editingAlias && (
            <label className="block font-semibold text-gray-700 dark:text-gray-200">
              ...
              <select
                ...
                onChange={(event) =>
                  setAliasForm((current) => ({ ...current, status: event.target.value }))
                }
              >
                <option value="ACTIVE">사용</option>
                <option value="INACTIVE">미사용</option>
