...
이 애플리케이션은 Excel/CSV 기반 업무 자동화를 위한 데스크톱 워크스페이스입니다. 파일 업로드부터 데이터 확인, 자동화 실행, 검증 결과 확인, 로그 추적까지 하나의 화면 흐름 안에서 처리할 수 있도록 구성했습니다.

| 영역 | 구현 내용 |
| --- | --- |
| Dashboard | 현재 작업 파일, 자동화 진행 상태, 검증 결과를 한눈에 확인하는 첫 화면 |
| Excel-like Grid | 행 번호, 시트 탭, 고정 헤더, 상태 칩을 포함한 스프레드시트형 데이터 테이블 |
| File Upload | CSV/XLSX 파일 업로드 후 첫 번째 시트를 자동으로 테이블 데이터로 표시 |
| Sample Data | 기본 화면에서 1,200건의 테스트 데이터로 긴 테이블 UI 확인 가능 |
| Automation Queue | 데이터 정리, 코드 매핑, 중복 검사, 보고서 생성 등 자동화 흐름 표시 |
| Workspace Explorer | 최근 파일, 고정 파일, 백업 버전을 한쪽 패널에서 관리 |
| Logs & Status | 실시간 작업 로그, 오류/경고, 선택 셀, 인코딩, 자동 저장 상태 표시 |
| Module Pages | 파일 관리, 자동화, 데이터 테이블, 백업, 설정, 관리자 메뉴로 확장 가능한 구조 |

## 화면 구성

첫 화면은 단순한 템플릿 대시보드가 아니라, 실제 업무자가 바로 사용할 수 있는 작업 공간처럼 보이도록 설계했습니다.
...
이번 작업의 핵심은 기존 템플릿형 카드 대시보드를 실제 업무용 Excel/CSV 자동화 워크스페이스로 재구성하는 것이었습니다.

- 템플릿 중심의 화면을 실무자가 바로 사용할 수 있는 작업 구조로 변경
- 여러 메뉴가 동일한 완성도로 보이도록 공통 업무 화면 컴포넌트 구성
- 파일 탐색기, 자동화 큐, 로그, 상태 바 등 데스크톱 업무 앱에 필요한 흐름 설계
- 비개발자 사무 사용자가 쉽게 이해할 수 있는 정보 구조 우선 적용
- 데이터 확인, 자동화 실행, 결과 검토가 자연스럽게 이어지는 사용 경험 구현
