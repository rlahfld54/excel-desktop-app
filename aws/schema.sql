...
CREATE TABLE IF NOT EXISTS customer_alias_mappings (
  ...
  CONSTRAINT uq_customer_alias_source
    UNIQUE (
      source_type,
  ...
  CONSTRAINT ck_customer_alias_source_value
    CHECK (
      NULLIF(TRIM(source_customer_code), '') IS NOT NULL
      OR NULLIF(TRIM(source_customer_name), '') IS NOT NULL
    )
);

ALTER TABLE contacts ADD COLUMN IF NOT EXISTS sync_key TEXT;
CREATE UNIQUE INDEX IF NOT EXISTS idx_contacts_sync_key ON contacts(sync_key);
...
CREATE TABLE IF NOT EXISTS customer_alias_mappings (
  ...
  CONSTRAINT fk_customer_alias_customer
